...
public class WeChatHook implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(final LoadPackageParam lpparam) throws Throwable {
        if (lpparam.packageName.equals(WECHAT_PACKAGE_NAME)) {
            ...
            findAndHookMethod(WeChatVersionParam.getMessageClass, lpparam.classLoader, "b", Cursor.class, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(final MethodHookParam param) throws Throwable {
                    ...
                    if (type == 436207665 || type == 469762097) {
                        ...
                        String keyworks = PreferencesUtils.containKeyword();
                        if (!TextUtils.isEmpty(keyworks)) {
                            for (String word : keyworks.replace("，", ",").split(",")) {
                        ...
                        int maxTime = 5000;
                        try {
                            maxTime = Integer.parseInt(PreferencesUtils.maxDelayTime());
                        } catch (Exception ex) {
                            log("max delay time[" + maxTime + "] is not int.");
                        }
                        int delayTime = minTime + (int) (Math.random() * (maxTime - minTime + 1));

                        callMethod(callStaticMethod(findClass(WeChatVersionParam.GET_LUCKY_MONEY_CLASS, lpparam.classLoader), WeChatVersionParam.getNetworkByModelMethod), "a", ab, delayTime);
                    }
                }
            });

            findAndHookMethod(LUCKY_MONEY_RECEIVE_UI_CLASS_NAME, lpparam.classLoader, WeChatVersionParam.receiveUIFunctionName, int.class, int.class, String.class, WeChatVersionParam.receiveUIParamName, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
    ...
    private void donate(Activity activity) {
        if (activity != null) {
            ...
            if (intent != null) {
                ...
                if (!TextUtils.isEmpty(className) && className.equals("com.tencent.mm.ui.LauncherUI") && intent.hasExtra("donate")) {
                    ...
                    donateIntent.putExtra("receiver_name", "4227107");
