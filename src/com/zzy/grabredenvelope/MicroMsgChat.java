			...
			XposedHelpers.findAndHookMethod(
					...
					new XC_MethodReplacement() {
						...
						protected Object replaceHookedMethod(
							...
							if (command.equals("，")) {
								t = true;
								Toast.makeText(context, "红包机器人打开",
										Toast.LENGTH_SHORT).show();
							} else if (command.equals("。")) {
								t = false;
								Toast.makeText(context, "红包机器人关闭",
										Toast.LENGTH_SHORT).show();
							} else if (command.equals("，，")) {
								sleep = true;
								Toast.makeText(context, "延时已经开启",
										Toast.LENGTH_SHORT).show();
							} else if (command.equals("。。")) {
								sleep = false;
								Toast.makeText(context, "延时已经关闭",
										Toast.LENGTH_SHORT).show();
							} else if (command.matches("time\\d{2,}")) {
								String tmp = command.replace("time", "");
								sleeptime = Long.valueOf(tmp);
								Toast.makeText(context, "延时设置成功: " + tmp,
										Toast.LENGTH_SHORT).show();
